...
componentInstallers['handleable middleware'] = middleware.installHandleableMiddlewareComponent
...
componentInstallers['handleable filter'] = middleware.installHandleableFilterComponent
...
var installComponent = function(componentSpec, callback) {
  ...
  var err = validate.validateComponentSpec(componentSpec)
  if(err) return callback(err)

  var componentType = componentSpec.type

  if(!componentInstallers[componentType]) {
    return callback(error(500, 'invalid component type ' + componentType))
  }
