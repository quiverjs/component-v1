...
var metaMiddlewareInstaller = function(handleableToHandlerMiddleware) {
  var installMiddlewareComponent = function(componentSpec, callback) {
    var err = validate.validateMiddlewareComponentSpec(componentSpec)
    if(err) return callback(err)
...
var metaFilterInstaller = function(installMiddlewareComponent) {
  var installFilterComponent = function(componentSpec, callback) {
    var err = validate.validateFilterComponentSpec(componentSpec)
    if(err) return callback(err)

    var filterName = componentSpec.name
    var filter = componentSpec.filter
    if(!filter) return callback(error(400, 'invalid filter ' + filterName))
    ...
    componentSpec.middleware = convertedMiddleware

    installMiddlewareComponent(componentSpec, callback)
  }
