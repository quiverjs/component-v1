...
var createSimpleHandleableBuilderFromComponentSpec = function(componentSpec, callback) {
  ...
  if(componentSpec.handler) {
    if(typeof(componentSpec.handler) != 'function') return callback(error(400,
      'simple handler component may not extend from other component'))

    simpleHandlerBuilder = createHandlerBuilderFromHandler(componentSpec.handler)
  }
